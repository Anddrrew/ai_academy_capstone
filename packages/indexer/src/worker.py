...
from store import store, IndexRecord, FileType
...
class Worker:
    ...
    def _process(self, record: IndexRecord) -> None:
        if record.file_type == FileType.FILE:
            self._process_file(record)
        elif record.file_type == FileType.GITHUB:
            self._process_github(record)
    ...
    def _process_github(self, record: IndexRecord) -> None:
        raise NotImplementedError(f"GitHub indexing not yet implemented: {record.path}")
